...
class App extends Component {
    ...
    onKeyPress = (button) => {
        ...
        if (button === "{enter}") {
            const { keyboard,user} = this.state;
            keyboard.visible=false;
            if (keyboard.role==="Name"){
                user.name=keyboard.text;
                DEBUG && console.log('user name',user.name);
            }
            else if (keyboard.role==="Project Name"){
                user.project=keyboard.text;
                DEBUG && console.log('Project name',user.project);
            }
            else if (keyboard.role==="Vehicle Number"){
                user.vehicleNo=keyboard.text;
                DEBUG && console.log('Vehicle No',user.vehicleNo);
                
            } 
            else if (keyboard.role==="ODC Number"){
                user.ODC=keyboard.text;
                DEBUG && console.log('ODC',user.ODC);
        
            }

            socket.emit('event-details-received',keyboard.role);
            keyboard.text="";
            keyboard.role='';

            this.setState({user,keyboard});
        }
    }
    componentDidMount() {
        ...
        //ask vehicle no
        socket.on('event-get-details', (role) => {
            ...
            keyboard.role =role;
            keyboard.title=role;
            keyboard.placeholder='Enter '+ role;
            this.setState({ keyboard });
        });
        //Ask Name
        socket.on('event-fetch-vehicle-details', () => {
            const { user } = this.state;
            socket.emit('event-process-vehicle-details',{name:user.name,empid:user.empid,vehicleNo:user.vehicleNo});
            DEBUG && console.log('Processing vehicle details');
